...
function mapStatus(apiStatus) {
    ...
    // A timed-out refresh IS a terminal failure. Mapping it to 'unknown' left the
    // record neither active nor finished, so the failure never showed in history.
    if (s === 'timedout') return 'failed';
    if (s === 'cancelled' || s === 'disabled') return 'cancelled';
    // 'Cancelling' is transitional — still running until Power BI reports Cancelled.
    if (s === 'inprogress' || s === 'notstarted' || s === 'cancelling') return 'inProgress';
